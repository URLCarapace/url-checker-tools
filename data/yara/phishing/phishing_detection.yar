...
rule Context_Aware_Brand_Impersonation
{
    ...
    strings:
        ...
        // Gmail/Google impersonation patterns (non-Google domains)
        $gmail_impersonation = /gmail[a-zA-Z0-9\-]*(?:support|help|security|account|center)/i
        $google_impersonation = /google[a-zA-Z0-9\-]*(?:support|help|security|account|center)/i

        // Brand + urgency + credential request
        $brand_urgent1 = /(paypal|amazon|microsoft|google|apple|gmail).*(?:urgent|expire|suspend).*(?:verify|login|update)/i
        $brand_urgent2 = /(account.*suspended|unauthorized.*access).*(?:paypal|amazon|microsoft|google|apple|gmail)/i

    condition:
        not any of ($legit*) and
        (any of ($fake_*) or any of ($typo_*) or any of ($brand_urgent*) or $gmail_impersonation or $google_impersonation)
}
